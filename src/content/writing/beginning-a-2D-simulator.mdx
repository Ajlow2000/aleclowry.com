...
import buzzImg from '../assets/buzz-lightyear-meme.jpeg';
import fleetsimMovementV0 from '../assets/fleetsim-movement-v0.mp4';
...
I was/am a huge Star Wars fan and for a kid like me, a game that requires memorizing the names of various ships, their roles in a fleet and what other ships they were strong/weak against was perfect.
Watching Imperial Star Destroyers maneuver around the map after being issued orders was the coolest thing ever.
Fast forward to college: I had my first PC capable of playing some games and I finally got the Steam edition of SW:EaW and was able to bask in its mod support that continues to keep that game alive.
...
Now to address the article description: I interviewed with a company that asked me to complete a take home project.
...
But it was a cool project that got me thinking a lot about how to design a deterministic simulator/state machine thing with testing as a primary goal (in my job as a SWE I largely maintain existing projects where this isn't the case).
...
The initial take-home project was interesting enough that I was curious about revisiting it and fixing some of the problems I created for myself with the first implementation.
Add in the idea of using this base to explore a space naval RTS game inspired by SW:EaW, and I was hooked.
...
Three important guiding philosophies I have for this simulator:
...
For now I'll just have a basic test client that moves some simple shapes around so I can explore the simulator's behavior.
...
<Image src={buzzImg} alt="I swear I'm unique" />

BUT- I don't have super technical reasons for this.
I write Rust in my day job, so it's just the language I am most comfortable with.
I think Rust/Bevy is a good fit for me though because I want as much of this project to live in source code as possible.
...
I've found that (while still valuable) unit tests often act as "state capture" of developer intentions when they implement the code and it's not until that code gets deployed through an actual application that real bugs are found.
One thing I'm *not* excited about is spending lots of time reading through log files to deconstruct what led to a weird behavior that I observed only after clicking around in a client.
...
In getting this project stood up I implemented a very basic movement system that tracks a unit's position (x,y coordinates) and accepts a move order to a destination coordinate.
...
<video autoplay loop muted playsinline width="100%">
  <source src={fleetsimMovementV0} type="video/mp4" />
</video>
...
fn three_dest() {
    ...
    // This is the autogenerated line that ensures the test fails until removed and replaced with something intentional
...
For the time being-- I think it's pretty cool though and a good base to build upon.
